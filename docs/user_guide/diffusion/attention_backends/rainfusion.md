...
## Wan2.2 T2V precision and dense fallback
...
`fp8_rotate_quant_bsa` runtime symbol, Ascend A5 (950), and its required dtype
and operators. Missing capabilities follow the configured precision fallback
before quantization executes. BF16 keeps the existing `rf_v2` route.
